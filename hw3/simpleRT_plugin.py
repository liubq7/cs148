...
def RT_trace_ray(scene, ray_orig, ray_dir, lights, depth=0):
    ...
    has_hit, hit_loc, hit_norm, _, hit_obj, _ = ray_cast(
        scene, ray_orig, ray_dir)
    ...
    for light in lights:
        ...
        modify_loc = hit_loc + eps * hit_norm
        light_vec = light.location - modify_loc
        ...
        #
        color += np.array(diffuse_color) * np.array(I_light) * \
            max(light_dir.dot(hit_norm), 0.0)
        #
    ...
    if depth > 0:
        ...
        if mat.transmission > 0:
            ...
            if ray_inside_object:
                n1, n2 = n2, n1
            ratio = n1 / n2
            cos = hit_norm.dot(-ray_dir)
            square = 1.0 - pow(ratio, 2.0) * (1.0 - pow(cos, 2.0))
            if square > 0:
                modify_neg_loc = hit_loc - eps * hit_norm
                D_transmit = ray_dir * ratio + \
                    hit_norm * (ratio * cos - sqrt(square))
                L_transmit = RT_trace_ray(
                    scene, modify_neg_loc, D_transmit, lights, depth-1)
                # add transmission to the final color: (1 - k_r) * L_transmit
                color += (1.0 - reflectivity) * mat.transmission * L_transmit
    #
